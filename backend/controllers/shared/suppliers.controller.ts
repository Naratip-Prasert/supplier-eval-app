    ...
    const result = await pool.query(
      isAdmin
        ? `SELECT vendor_code AS "vendorCode", supplier_name AS "supplierName",
             ...
             FROM "SPES2_suppliers" ORDER BY supplier_name`
        : `SELECT vendor_code AS "vendorCode", supplier_name AS "supplierName",
                  product_type AS "productType"
             FROM "SPES2_suppliers" WHERE is_active = TRUE ORDER BY supplier_name`
    );
...
async function createSupplier(req: Request, res: Response) {
  ...
  try {
    const result = await pool.query(
      `INSERT INTO "SPES2_suppliers" (vendor_code, supplier_name, product_type, is_active)
       VALUES ($1, $2, $3, TRUE)
...
async function checkPermission(req: Request, res: Response) {
  ...
  try {
    ...
    const [empResult, supResult] = await Promise.all([
      pool.query(
        `SELECT e.emp_no AS id, COALESCE(r.role, 'USER') AS role FROM "Master_Data_All" e
         LEFT JOIN "SPES2_Roles" r ON e.emp_no = r.emp_no
    ...
    const permResult = await pool.query(
      `SELECT 1 FROM "SPES2_employee_supplier_permissions"
