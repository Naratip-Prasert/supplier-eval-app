      ...
      [categoriesResult, criteriaResult, levelsResult] = await Promise.all([
        ...
        pool.query(
          `SELECT id, category_id AS "categoryId", code,
             ...
             FROM "SPES2_evaluation_sub_criteria"
        ...
        pool.query(
          `SELECT criterion_id AS "criterionId", level, description
             FROM "SPES2_score_level_descriptions"
...
async function deleteCategory(req: Request, res: Response) {
  ...
  try {
    ...
    await client.query(
      `UPDATE "SPES2_evaluation_sub_criteria" SET is_active = FALSE WHERE category_id = $1`,
      [id]
    );
    const result = await client.query(
      `UPDATE "SPES2_evaluation_main_criteria" SET is_active = FALSE WHERE id = $1 RETURNING id`,
      [id]
    );
...
async function updateCategory(req: Request, res: Response) {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE "SPES2_evaluation_main_criteria"
  ...
  try {
    ...
    if (type === 'function') {
      ...
      if (inactiveRes.rows.length > 0) {
        ...
        await client.query(
          `UPDATE "SPES2_evaluation_main_criteria" SET is_active=TRUE, name_th=$1, total_weight=$2 WHERE id=$3`,
      ...
      const { rows } = await client.query(
        `SELECT code FROM "SPES2_evaluation_main_criteria" WHERE code LIKE $1 ORDER BY code`,
        [`${funcPrefix}%`]
      );
      ...
      while (true) {
        code = `${prefix}${n}`;
        const { rowCount } = await client.query('SELECT 1 FROM "SPES2_evaluation_main_criteria" WHERE code=$1', [code]);
        if (rowCount === 0) break;
        n++;
      }
    }
    ...
    if (isCoreSection) {
      ...
      const esgRes = await client.query(
        `SELECT display_order FROM "SPES2_evaluation_main_criteria" WHERE code = $1`,
        [esgCode]
      );
      if (esgRes.rows.length > 0) {
        const esgOrder = esgRes.rows[0].display_order;
        await client.query(
          `UPDATE "SPES2_evaluation_main_criteria" SET display_order = display_order + 1 WHERE code = $1`,
          [esgCode]
        );
        nextOrder = esgOrder;
      } else {
        ...
        const orderRes = await client.query(
          `SELECT COALESCE(MAX(display_order),0)+1 AS next FROM "SPES2_evaluation_main_criteria" WHERE code LIKE $1`,
          [orderLikePat]
        );
    ...
    const result = await client.query(
      `INSERT INTO "SPES2_evaluation_main_criteria" (code, name_th, total_weight, display_order, is_active)
       VALUES ($1,$2,$3,$4,TRUE)
...
async function createItem(req: Request, res: Response) {
  ...
  try {
    ...
    const categoryRes = await client.query(
      `SELECT is_active FROM "SPES2_evaluation_main_criteria" WHERE id = $1`,
      [categoryId]
    );
    ...
    const collisionRes = await client.query(
      `SELECT id, is_active FROM "SPES2_evaluation_sub_criteria" WHERE criteria_set = $1 AND code = $2`,
      [criteriaSet, newCode]
    );
    ...
    const siblingsRes = await client.query(
      `SELECT display_order AS "displayOrder" FROM "SPES2_evaluation_sub_criteria"
    ...
    if (siblingsRes.rows.length > 0) {
      ...
      await client.query(
        `UPDATE "SPES2_evaluation_sub_criteria" SET display_order = display_order + 1
          WHERE category_id = $1 AND display_order >= $2`,
        [categoryId, nextOrder]
      );
    } else {
      const orderRes = await client.query(
        'SELECT COALESCE(MAX(display_order), 0) + 1 AS next FROM "SPES2_evaluation_sub_criteria" WHERE category_id = $1',
        [categoryId]
      );
    ...
    const defaultLevels = Array.isArray(levels) && levels.length > 0
      ? levels
      : ['', '', '', '', ''];
    await client.query('DELETE FROM "SPES2_score_level_descriptions" WHERE criterion_id = $1', [newId]);
    for (let i = 0; i < defaultLevels.length; i++) {
      await client.query(
        'INSERT INTO "SPES2_score_level_descriptions" (criterion_id, level, description) VALUES ($1, $2, $3)',
...
async function deleteItem(req: Request, res: Response) {
  ...
  try {
    const result = await pool.query(
      'UPDATE "SPES2_evaluation_sub_criteria" SET is_active = FALSE WHERE id = $1 RETURNING id',
      [id]
    );
    if (result.rowCount === 0) return res.status(404).json({ message: 'ไม่พบข้อมูล' });
...
async function updateItem(req: Request, res: Response) {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE "SPES2_evaluation_sub_criteria"
          SET name_th        = COALESCE($1, name_th),
...
async function updateItemLevels(req: Request, res: Response) {
  ...
  try {
    ...
    if (levelValues !== undefined) {
      await client.query(
        'UPDATE "SPES2_evaluation_sub_criteria" SET level_values = $1 WHERE id = $2',
        [levelValues === null ? null : JSON.stringify(levelValues), id]
      );
    }
    await client.query('DELETE FROM "SPES2_score_level_descriptions" WHERE criterion_id = $1', [id]);
...
async function reorder(req: Request, res: Response) {
  ...
  try {
    ...
    for (const { id, displayOrder } of order) {
      await client.query(
        'UPDATE "SPES2_evaluation_main_criteria" SET display_order = $1 WHERE id = $2',
        [displayOrder, id]
      );
    }
    await client.query('COMMIT');
...
async function seed(req: Request, res: Response) {
  ...
  try {
    ...
    for (const section of sections) {
      ...
      if (reset) {
        const catRes = await client.query(
          `INSERT INTO "SPES2_evaluation_main_criteria" (code, name_th, total_weight, display_order)
           VALUES ($1, $2, $3, $4)
        ...
        catId = catRes.rows[0]
          ? catRes.rows[0].id
          : (await client.query('SELECT id FROM "SPES2_evaluation_main_criteria" WHERE code=$1', [section.code])).rows[0].id;
      }
    ...
    if (reset && newCritIds.length > 0) {
      await client.query(
        'DELETE FROM "SPES2_score_level_descriptions" WHERE criterion_id = ANY($1::uuid[])',
        [newCritIds]
      );
    }
