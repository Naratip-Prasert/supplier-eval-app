// ============================================================
//  pages/SupplierFeedbackPage.jsx
...
const ROLE_LABEL = { USER: "ผู้ประเมิน (ฝ่ายใช้งาน)", GCP: "ผู้จัดซื้อ (Buyer)" };
...
export default function SupplierFeedbackPage({ token }) {
  const [state, setState]     = useState("loading"); // loading | error | ready | submitted
  const [errorMsg, setErrorMsg] = useState("");
  const [data, setData]       = useState(null);
  const [scores, setScores]   = useState({}); // { [employeeId]: { [code]: score } }
  ...
  const setScore = (employeeId, code, value) => {
