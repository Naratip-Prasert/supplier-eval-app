...
export const AHP_MAIN_W: Record<string, number>  = { '1': 38.65, '2': 21.21, '3': 21.21, '4': 11.98, '5': 6.94 };
export const AHP_LOCAL_W: Record<string, number> = {
...
export function getAhpMain(section: CriteriaSection): number | null {
  const first = section.items.find(i => !i.divider);
  if (!first || !first.no) return null;
  return AHP_MAIN_W[first.no.charAt(0)] ?? null;
}
export function getAhpLocal(itemNo: string | null | undefined): number | null {
...
export function esgGroupKey(label: string | null | undefined): string | null {
...
export function assignEsgSubGroupWeights(
  sectionItems: CriteriaEntry[],
  sw: number,
  items: Record<string, number>,
  groupPct?: Record<string, number> | null
): void {
  interface Group { key: string | null; fallbackPct: number | null; items: CriteriaEntry[] }
  const groups: Group[] = [];
  let curKey: string | null = null, curFallbackPct: number | null = null, curItems: CriteriaEntry[] = [];
  const pushGroup = () => {
...
export function initWeights(criteria: CriteriaSection[]): { sections: Record<number, number>; items: Record<string, number> } {
  ...
  criteria.forEach((section, si) => {
    ...
    if (realItems.every(i => i.weight != null) && Math.abs(constantSum - sw) < 0.1) {
      realItems.forEach(item => { if (item.no) items[item.no] = item.weight!; });
    ...
    // 2. AHP proportions: use AHP_LOCAL_W percentages when available for all items
      ...
      const ahpSum = realItems.reduce((s, i) => s + (getAhpLocal(i.no) ?? 0), 0);
      let iRem = sw;
      realItems.forEach((item, ii) => {
        if (!item.no) return;
        if (ii === realItems.length - 1) { items[item.no] = Math.max(0, r2(iRem)); }
        else { const w = r2(sw * (getAhpLocal(item.no) ?? 0) / ahpSum); items[item.no] = w; iRem -= w; }
      });
    ...
    // 3. Proportional from item.weight (handles constants.js weights on different scale)
      ...
      realItems.forEach((item, ii) => {
        if (!item.no) return;
        if (ii === realItems.length - 1) { items[item.no] = Math.max(0, r2(iRem)); }
        else { const w = r2(((item.weight ?? 0) / constantSum) * sw); items[item.no] = w; iRem -= w; }
      });
