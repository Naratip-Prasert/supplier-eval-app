...
// ── Pill filter button ────────────────────────────────────────
function Pill({ active, onClick, children, color }: { active: boolean; onClick: () => void; children: ReactNode; color?: string }) {
  return (
    <button
      onClick={onClick}
      style={{
...
export default function HistoryPage() {
  ...
  const onViewDetail = (id: string) => router.push(`/evaluations/${id}?from=history`);
  ...
  const gradeCounts = useMemo(() => {
    ...
    records.forEach(r => { if (r.grade && r.grade in counts) counts[r.grade]++; });
    return counts;
  }, [records]);
  ...
  const handleEvalFilter = (v: string) => {
    setFilterEval(v);
