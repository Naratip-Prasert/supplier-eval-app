...
export function Header({ subtitle, backLabel, onBack, titleOverride, user, onLogout, profilePic }: HeaderProps) {
...
export function CustomSelect({ label, required, options, value, onChange, disabled }: CustomSelectProps) {
...
type ModalState = { type: "alert" | "confirm"; title: string; message: string } | null;
...
function AppModal({ type, title, message, onClose }: AppModalProps) {
...
export function useModal() {
  ...
  const showAlert = (message: string, title = "แจ้งเตือน"): Promise<boolean | undefined> =>
    new Promise((resolve) => {
      resolveRef.current = resolve;
      setState({ type: "alert", title, message });
    });

  const showConfirm = (message: string, title = "ยืนยัน"): Promise<boolean | undefined> =>
    new Promise((resolve) => {
      resolveRef.current = resolve;
      setState({ type: "confirm", title, message });
    });

  const handleClose = (value: boolean | undefined) => {
