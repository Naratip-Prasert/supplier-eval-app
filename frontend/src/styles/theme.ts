...
export const ROLE_THEME: Record<string, { main: string; bg: string; gradient: string }> = {
...
export function roleThemeColor(role: string | null | undefined): string {
  return ROLE_THEME[role?.toUpperCase() ?? '']?.main ?? ROLE_THEME.USER.main;
}

export function roleThemeGradient(role: string | null | undefined): string {
